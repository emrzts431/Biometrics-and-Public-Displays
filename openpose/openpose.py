...
"""
Number of participants. Number of repititions for all participants should be 10
"""
NUM_PARTICIPANTS = 3
...
current_participant_rep = 0
for f in image_folder_list:
    participant_name = f'PID_{current_participant}'
    ...
    if not os.path.exists(rf'{logs_write_folder}\{participant_name}'):
        os.mkdir(rf'{logs_write_folder}\{participant_name}')
    num_folders = len(os.listdir(rf'{logs_write_folder}\{participant_name}'))
    openpose_path = rf'{logs_write_folder}\{participant_name}\{0 if num_folders == 0 else num_folders}\openpose'
    if not os.path.exists(rf'{logs_write_folder}\{participant_name}\{0 if num_folders == 0 else num_folders}'):
        os.mkdir( rf'{logs_write_folder}\{participant_name}\{0 if num_folders == 0 else num_folders}')
        os.mkdir(rf'{logs_write_folder}\{participant_name}\{0 if num_folders == 0 else num_folders}\openpose')
        os.mkdir(rf'{logs_write_folder}\{participant_name}\{0 if num_folders == 0 else num_folders}\openpose\json')
        os.mkdir(rf'{logs_write_folder}\{participant_name}\{0 if num_folders == 0 else num_folders}\openpose\image')
    ...
    #endregion
    logfile = rf'{logs_write_folder}\{participant_name}\{0 if num_folders == 0 else num_folders}\logs.tsv'
    with open(logfile, 'w') as f:
        ...
        for frame in frames:
            ...
            #endregion
        print(len(frames))
        if current_participant_rep == 9:
            current_participant_rep = 0
            current_participant += 1
        else:
            current_participant_rep += 1
